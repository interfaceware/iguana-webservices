...
local function LoadHtml()
   local F = io.open(iguana.project.root()..'/'..iguana.project.guid()..'/index.html', "r")
   local Content = F:read("*a")
   F:close()
   return Content
end

DefaultHtml = LoadHtml()
...
function main(Data)
   ...
   if Request.location == '/fhir/' then
      net.http.respond{body=DefaultHtml}
      return
   end
