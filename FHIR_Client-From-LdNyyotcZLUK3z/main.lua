...
function main()
   ...
   if RunTests then
      ...
      local Patient1XmlString, XmlReadCode, XmlReadHeaders  = net.http.get{url= UrlBeginning .. "fhir/patient/1", parameters={_format = "xml"}, live=RunTests}
      trace(Patient1XmlString)
      trace(XmlReadCode)

      -- Perform a get request for patient with id = 1 (default format is json)
      local Patient1JsonString, JsonReadCode, JsonReadHeaders = net.http.get{url= UrlBeginning .. "fhir/patient/1", live=RunTests}
      ...
      -- Perform an update operation using a put request to update the patients info on the server using the altered json data.
      local UpdateResult, UpdateCode, UpdateHeaders = net.http.put{url= UrlBeginning .. "fhir/patient/1", data=NewPatient1JsonString, live=RunTests}

      --  A response code of 200 means the update was successful
      trace(UpdateCode)
      
      -- Will be an operation result explaining that the "Resource entry successfully updated"
      trace(UpdateResult)

      -----------------------------------
      --------- Search Operation --------
      -----------------------------------
      -- Perform a search operation that will retrieve all entries for the patient resource 
      -- (the result is a resource bundle containing all the results)
      local PatientBundleJsonString, SearchCode, SearchHeaders = net.http.get{url= UrlBeginning .. "fhir/patient/", live=RunTests}
      ...
      local PostResult, PostCode, PostHeaders = net.http.post{url= UrlBeginning .. "fhir/patient/", body=FhirPatients.json,  live=RunTests}
      ...
      -- that the "Resource entry successfully created" (or error if something went wrong)
      trace(PostResult)

      -- The new resource can be found at the location specified by the location header. (if success)
      ...
      local DeleteResult, DeleteCode, DeleteHeaders = net.http.delete{url=PostHeaders.Location,  live=RunTests}

      --  A response code of 204 means the resource was successfully removed or never existed
      trace(DeleteCode)
